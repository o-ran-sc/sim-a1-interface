...
echo "=== Put an a1 policy: shall return time-out ==="
req_id=$(get_random_number)
res=$(cat jsonfiles/timeout_response.json)
RESULT="json:$res"
# asynch callout
do_curl PUT  /policytypes/ptype1/kafkadispatcher/alpha 500 jsonfiles/alpha_policy.json $req_id &
proc_id=$!
sleep 35
# after 35 seconds, publish the event
publish_event $req_id
# wait until the main process to be completed
wait $proc_id

# asynch success test case after 10s
echo "=== Put an a1 policy: shall succeed ==="
req_id=$(get_random_number)
RESULT=""
# asynch callout
do_curl PUT  /policytypes/ptype1/kafkadispatcher/alpha 200 jsonfiles/alpha_policy.json $req_id &
proc_id=$!
sleep 10
...
echo "=== Delete an a1 policy: shall succeed ==="
req_id=$(get_random_number)
RESULT=""
# asynch callout
do_curl DELETE  /policytypes/ptype1/kafkadispatcher/alpha 200 jsonfiles/alpha_policy.json $req_id &
proc_id=$!
publish_event $req_id
# wait until the main process to be completed
wait $proc_id
