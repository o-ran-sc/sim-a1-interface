...
from maincommon import create_kafka_producer, create_kafka_consumer, create_kafka_event, byte_to_str, get_random_string
...
def put_policy(policyTypeId, policyId):
  ...
  kafka_event = create_kafka_event(policy_type_id, policy_id, data, 'PUT')
  if (MSG_BROKER_URL is not None):
    return publish_and_consume(kafka_event, req_id_from_header)
...
def publish_and_consume(kafka_evet, req_id_from_header):
  ...
  try:
    ...
    fut_rec_metadata = producer.send(KAFKA_TOPIC_REQ, kafka_evet, req_id)
    record_metadata = fut_rec_metadata.get()
    print('Future:', record_metadata)
    publish_time_in_ms = record_metadata.timestamp

    # For test purposes only, publish the same event to response topic
    producer.send(KAFKA_TOPIC_RES, kafka_evet, req_id)

    # synch-consume
    consumer_record = consume_record_for(req_id)
...
def consume_record_for(req_id):

  try:
    print ('req_id looking for in consumer:', req_id)
    consumer = create_kafka_consumer()
    topic_partition = TopicPartition(KAFKA_TOPIC_RES, 0)
...
def create_error_response(code):
    ...
    if code == 400:
      ...
      return(create_problem_json(None, "Publish failed", 419, "Publishing the event could not be processed on the Kafka cluster", None))
    elif code == 429:
