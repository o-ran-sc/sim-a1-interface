...
def get_policy_type_to_topic_mapping(policyTypeId):
  ...
  if policy_type_id in map_in_dict.keys():
    ...
    return Response(topic_address, 200, mimetype=TEXT_PLAIN)
  else:
...
def publish_and_consume(kafka_event, req_id_from_header, pol_type_id):
  ...
  try:
    ...
    if (resp.status_code == 404):
      ...
      target_topic = resp.data
      target_topic = byte_to_str(target_topic)

    # synch-publish
    # KafkaProducer.send(topicname, value=broker_message, key=req_id, headers=None, partition=None, timestamp_ms=None)
    fut_rec_metadata = producer.send(target_topic, kafka_event, req_id)
    ...
    # else comment out this line for the baseic_test.sh
    producer.send(KAFKA_TOPIC_RES, kafka_event, req_id)
