...
Build and start the simulator container version STD_2.0.0, using two alternative: ext-srv or kafka-srv. However, both can not be used at the same time to start A1 sim.
...
STD_2.0.0 version is also including an kafka message dispatcher that is a Python server building RESTful API. The kafka server supports HTTP/HTTPS protocols.
The description of the start parameters are explained below:
